...
class DispensationService{
  ...
  var data = [];
  List<DispensationModele> results = [];

  String fetchUrl ='$masante/dispensation/afficher';
  Future<List<DispensationModele>> getDispensationModel({String ,query}) async{
    final headers = {
    ...
    try{
      if(response.statusCode == 200){
        ...
        results = data.map((e)=> DispensationModele.fromJson(e)).toList();
  ...
  static Future<String> addDispensation(String nom, String prenom,String username,String email, String telephone,String password) async {
  ...
  static Future<List<DispensationModele>> getAllDispensation() async {
    //SharedPreferences prefs = await SharedPreferences.getInstance();
    List<DispensationModele> items = [];
    //String? token = prefs.getString("token");
    //Get the item from the API
    var url = Uri.parse('http://127.0.0.1:8082/dispensation/afficher');
    // var headers = {
    ...
    if (response.statusCode == 200) {
      ...
      items = data.map((e) => DispensationModele.fromJson(e)).toList();
      print("items");
      print(items);
